...
export function ContentUpload({userId, storeId, onSuccess}: ContentUploadProps) {
    ...
    const uploadFile = async (file: File) => {
        ...
        // --- FIX APPLIED HERE ---
        // The filePath now correctly starts with the user's ID,
        // which satisfies the RLS policy.
    ...
    return (
        // Card is already themed
        <Card className="w-full max-w-2xl mx-auto">
            <CardHeader/>
            <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                    {/* THEME: Themed the dropzone area */}
                    <div
                        {...getRootProps()}
                        className={cn(
                    ...
                    {files.length > 0 && (
                        <div className="space-y-2">
                            ...
                            {files.map((file, index) => (
                                // THEME: Themed the selected file list item
                                <div key={index}
                    ...
                    {error && (
                        // THEME: Themed the error message
